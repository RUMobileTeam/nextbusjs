  ...
  function customPredict(routeStops, cb) {
    ...
    query("predictionsForMultiStops", str, (error, response) => {
      try {
        ...
        const predictions = predictionContainers._toArray()
          .map((predictionContainer) => {
            ...
            return predictionContainer.getElementsByTagName("direction")
              ._toArray()
              .map((directionNode) => {
                const directionTitle = directionNode.getAttribute("title");
                const predictions = directionNode.getElementsByTagName("prediction")
                  ._toArray()
                  .map((predictionNode) => {
